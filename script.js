...
function render() {
  ...
  if (cigaretteLit && cigaretteRemaining > 0) {
    ...
    if (cigaretteRemaining > 0) burning.style.backgroundColor = "#77726d";
    else burning.style.opacity = "0";
    clearInterval(cigaretteInterval);
    blemBurning.pause();
    smoke.style.opacity = "0%";
  }

  if (zippoOpen) zippoLid.classList.add("open");
  else zippoLid.classList.remove("open");

  if (zippoLit) zippoFlame.style.opacity = 1;
  else zippoFlame.style.opacity = 0;
...
function toggleZippoLid() {
  zippoOpen = !zippoOpen;
  if (zippoOpen) zippoOpenSound.play();
  else zippoCloseSound.play();
  if (zippoOpen == false) zippoLit = false;
  render();
}
